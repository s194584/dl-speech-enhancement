...
path_to_config = os.path.join("config", "denoise", args.config)
...
split = [0.7, 0.15, 0.15]
train_clean_dataloader, val_clean_dataloader, _ = get_dataloaders(
    clean_dataset, split, batch_size, batch_length, SEED
)
train_noise_dataloader, val_noise_dataloader, _ = get_dataloaders(
    noise_dataset, split, batch_size, batch_length, SEED
)
...
for epoch in range(EPOCHS):
    ...
    print(f"Epoch {epoch}")
    for i_batch, (clean_batch, noise_batch) in enumerate(
